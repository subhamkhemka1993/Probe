...
class ProbeStartupInitializerTest {
    ...
    /**
     * The [ProbeStartupInitializer] only registers a hook — nothing has called
     * [ProbeRuntime.initialize] yet. Calling `install()` here plays the role a host app's own
     * install call plays in production; observing [ProbeRuntime.isEnabled] flip to true (driven
     * by the config passed below) is the only way to confirm the initializer's hook actually
     * reached `ProbeRuntime.initialize` rather than being silently dropped.
     */
    @Test
